...
# ==================== FULL APPENDED FIXES (no removals) ====================
# Alt levels render, star-wrapped main line, downtime compensation, schema guards, late DB retry.
import json as __json_fix, re as __re_fix
import aiosqlite as __aiosqlite_fix
import discord as __discord_fix
import asyncio as __asyncio_fix

def __coerce_lvl_fix(v):
    try:
        if v is None: return None
        if isinstance(v, int): return v
        m = __re_fix.search(r'\d+', str(v)); return int(m.group(0)) if m else None
    except Exception: return None

try:
    __orig_upsert_roster_fix = _upsert_roster  # noqa: F821
except NameError:
    __orig_upsert_roster_fix = None

async def _upsert_roster(gid, uid, main_name, main_level, main_class, alts, tz_raw, tz_norm):
    ml = __coerce_lvl_fix(main_level) or main_level
    alt_list = []
    try:
        if isinstance(alts, str): alt_list = __json_fix.loads(alts) or []
        elif isinstance(alts, list): alt_list = list(alts)
    except Exception: alt_list = []
    for a in alt_list:
        if isinstance(a, dict):
            a["level"] = __coerce_lvl_fix(a.get("level") or a.get("lvl") or a.get("lv"))
    if __orig_upsert_roster_fix:
        return await __orig_upsert_roster_fix(gid, uid, main_name, ml, main_class, alt_list, tz_raw, tz_norm)
    async with __aiosqlite_fix.connect(DB_PATH) as db:
        await db.execute("""CREATE TABLE IF NOT EXISTS roster_members (
            guild_id INTEGER NOT NULL,
            user_id INTEGER NOT NULL,
            main_name TEXT NOT NULL,
        ...
        await db.execute("""INSERT INTO roster_members
            ...
            ON CONFLICT(guild_id,user_id) DO UPDATE SET
                ...
                timezone_norm=excluded.timezone_norm,
                updated_at=excluded.updated_at
        """, (gid, uid, main_name, int(ml) if isinstance(ml,int) else ml, main_class, __json_fix.dumps(alt_list), tz_raw, tz_norm, gid, uid))
        await db.commit()

def _build_roster_embed(member, main_name, main_level, main_class, alts, tz_raw, tz_norm):
    try:
        if isinstance(alts, str):
            try: alts_list = __json_fix.loads(alts) or []
        ...
        for i, a in enumerate(alts_list, 1):
            if isinstance(a, dict):
                ...
                lv = __coerce_lvl_fix(a.get("level") or a.get("lvl") or a.get("lv"))
                cl = a.get("class","?")
            else:
                nm, lv, cl = "?", None, "?"
            lines.append(f"{i}. {nm} • {'Lv '+str(lv) if isinstance(lv,int) else 'Lv N/A'} • {cl}")
        mlv = __coerce_lvl_fix(main_level) or main_level
        main_line = f"**✨ {main_name} • Lv {mlv} • {main_class} ✨**"
        e = __discord_fix.Embed(title=f"New Member: {member.display_name}", color=__discord_fix.Color.blurple())
        e.add_field(name="Main", value=main_line, inline=False)
        ...
        return __discord_fix.Embed(title=f"New Member: {getattr(member,'display_name','?')}", description=f"{main_name} • {main_level} • {main_class}")

@bot.listen("on_ready")
async def __bind_embed_builder_fix():
    try:
        globals()["_build_roster_embed"] = _build_roster_embed
        if 'log' in globals(): log.info("[embed] enforced builder bound")
    except Exception: pass

async def __sqlite_schema_guard_fix():
    try:
        async with __aiosqlite_fix.connect(DB_PATH) as db:
        ...
        try:
            if 'log' in globals(): log.warning(f"[schema] guard failed: {e}")
        except Exception: pass

async def __downtime_comp_fix():
    try:
        boot = now_ts(); off_since = None
        try:
            v = await meta_get("offline_since")
            if v and str(v).isdigit(): off_since = int(v)
        except Exception: pass
        if off_since is None:
            try:
                ...
                if last and str(last).isdigit():
                    last_i = int(last)
                    if boot - last_i > CHECK_INTERVAL_SECONDS * 2: off_since = last_i
            except Exception: pass
        async with __aiosqlite_fix.connect(DB_PATH) as db:
            c = await db.execute("SELECT id,guild_id,channel_id,name,next_spawn_ts,category FROM bosses")
        ...
        try:
            if 'log' in globals(): log.warning(f"[boot] downtime comp failed: {e}")
        except Exception: pass

async def __db_retry_tune_fix():
    for _ in range(6):
        try:
            async with __aiosqlite_fix.connect(DB_PATH, timeout=10) as db:
                await db.execute("PRAGMA busy_timeout=10000;")
                await db.execute("PRAGMA journal_mode=WAL;")
                await db.execute("SELECT 1"); await db.commit(); return True
        except Exception: await __asyncio_fix.sleep(1)
    return False

@bot.listen("on_ready")
async def __run_full_appended_fixes():
    await __sqlite_schema_guard_fix()
    await __downtime_comp_fix()
    ok = await __db_retry_tune_fix()
    try:
        if 'log' in globals():
            if ok: log.info("[fixes] schema+downtime+tuner applied")
            else: log.warning("[fixes] tuner did not confirm")
    except Exception: pass
# ==================== END FULL APPENDED FIXES ====================
