...
def _window_segment_if_due(now: int, next_ts: int, window_m: int) -> str:
    """Return formatted window segment only when due (not pending).
    Extra checks: type guard, length guard, and pending filter.
    """
    try:
        label = window_label(now, next_ts, window_m)
    except Exception:
        return ""
...
async def _build_timer_embeds_compact(guild: dm.Guild, categories: _List[str]):
    ...
    for cat in categories:
        ...
        desc = "\n".join(lines)[:4096] if lines else "No timers."
        em = dm.Embed(
            title=f"{category_emoji(cat)} {cat}",
