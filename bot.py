...
class TimerToggleView(discord.ui.View):
    ...
    async def refresh(self, interaction: discord.Interaction):
        # persist and rebuild Select with current defaults
        try:
            await self.persist()
            try:
            ...
            try:
                ...
                _dm = None
            for _ch in list(getattr(self, 'children', []) or []):
                try:
                    if (_Select and isinstance(_ch, _Select)) or (_dm and isinstance(_ch, _dm.ui.Select)):
                        self.remove_item(_ch)
                except Exception:
                    pass
            try:
                MobileCategorySelect  # type: ignore
                self.add_item(MobileCategorySelect(self))  # type: ignore
            except Exception:
                pass
            for _ch in list(getattr(self, 'children', []) or []):
                _opts = getattr(_ch, 'options', None)
                if _opts:
                    for _opt in _opts:
                        try:
                            _opt.default = (_opt.value in getattr(self, 'shown', []))
                        except Exception:
                            pass
        except Exception:
            pass
...
# ---- Class-role normalization helpers (robust) ----
_CLASS_ROLE_COLUMNS = {
        ...
        if isinstance(alts, list):
            for a in alts:
                if isinstance(a, str):
                    ...
                    for k in ("class","Class","cls","value","label","name"):
                        if k in a:
                            c = _norm_class(a.get(k))
                            if c:
                                yield c
        elif isinstance(alts, dict):
            for k in ("class","Class","cls","value","label","name"):
                if k in alts:
                    c = _norm_class(alts.get(k))
                    if c:
                        yield c
    except Exception:
        return
...
@_ac_cfg.checks.has_permissions(manage_roles=True)
@_ac_cfg.describe(class_name="Ranger | Rogue | Warrior | Mage | Druid", role="Role mention")
...
@_ac_cfg.command(name="clear-class-role", description="Clear class mapping")
@_ac_cfg.checks.has_permissions(manage_roles=True)
@_ac_cfg.describe(class_name="Ranger | Rogue | Warrior | Mage | Druid")
...
@_ac_cfg.command(name="list-class-roles", description="List current class mappings")
@_ac_cfg.checks.has_permissions(manage_guild=True)
async def list_class_roles(interaction: discord.Interaction):
    ...
    for cname, field in _CLASS_ROLE_COLUMNS.items():
        ...
        if rid:
            ...
            lines.append(f"{cname}: {role.mention if role else rid}")
        else:
            lines.append(f"{cname}: not set")
